...
const COMPANY_ID = 1; // hardcoded for now — Phase 8 can make this dynamic
...
export default function DashboardPage() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await getBolHistory(COMPANY_ID);
      setBols(res.data);
    } catch {
  ...
  const handleGenerate = async () => {
    ...
    setError('');
    try {
      const res = await generateBol(COMPANY_ID);
  ...
  const handleVoid = async (bol: BolRecord) => {
    ...
    if (!reason) return;
    try {
      await voidBol(COMPANY_ID, bol.id, reason);
  ...
  const handleDownload = async (bol: BolRecord) => {
    try {
      await downloadPdf(COMPANY_ID, bol.id, bol.bolNumber);
    } catch {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-700 mb-4">Generate New BOL</h2>
